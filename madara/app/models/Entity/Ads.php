<?php
	...
	class Ads extends Models\Metadata {
		...
		public function get_ad( $ad_pos, $class = '' ) {
			...
			$extra_data = '';
			if($ad_pos == 'ads_wall_left' || $ad_pos == 'ads_wall_right'){
				$scrolltop = \App\Madara::getOption('ads_wall_scrolltop', 210);
				$maxtop = \App\Madara::getOption('ads_wall_maxtop', 510);

				$extra_data = ' data-scrolltop="' . $scrolltop . '" data-maxtop="' . $maxtop . '" ';
			}

			if ( $pub_id != '' && $slot_id != '' ) {
				$html .= '<div '. $extra_data . ' class="ad c-ads ' . esc_attr( $class ) . '">' . $this->get_gooogle_responsive_ad( $pub_id, $slot_id ) . '</div>';
			} else {
				$html .= \App\Madara::getOption( $ad_pos ) != '' ? '<div '. $extra_data . ' class="ad c-ads custom-code ' . esc_attr( $class ) . '">' . \App\Madara::getOption( $ad_pos ) . '</div>' : '';
			}
