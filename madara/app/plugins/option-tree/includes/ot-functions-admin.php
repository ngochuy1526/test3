					...
					$html_filter = function ( $tags, $context ) {
						if ( 'post' === $context ) {
							if ( current_user_can( 'unfiltered_html' ) || true === OT_ALLOW_UNFILTERED_HTML ) {
								$tags['script']   = array_fill_keys( array( 'async', 'charset', 'defer', 'src', 'type', 'data-*' ), true );
...
if ( ! function_exists( 'ot_decode' ) ) {
	...
	function ot_decode( $value ) {
		...
		// updated by wpstylish to fix "Add New" action for list_item
		preg_match( '/O:\d+:\"(.*?)\"/', $decoded, $obj_matches, PREG_OFFSET_CAPTURE, 0 );
