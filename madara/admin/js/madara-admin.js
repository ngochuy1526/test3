...
jQuery(document).ready(function (e) {
    ...
    if(jQuery('body').hasClass('post-type-page')){
        setTimeout(function () {
			...
			if(page_tpl_obj.length == 0){
				// for WP 6.1+
				page_tpl = jQuery('.edit-post-post-template__toggle').text();	
				if(page_tpl == ""){
					jQuery('.editor-post-panel__row-label').each(function(){
						if(jQuery(this).html() == 'Template'){
							var btn = jQuery(this).next().find('.components-button');
							if(btn){
								page_tpl = btn.html();
							}
						}
					})
					//page_tpl = jQuery('.editor-post-template__classic-theme-dropdown select').val();
				}
			}
            ...
            if (page_tpl == 'page-templates/front-page.php' || page_tpl == 'page-templates/fullpage.php' || page_tpl == 'Front-Page') {
                // do nothing
			...
			if(jQuery('.edit-post-post-template__toggle').length){
				// WP 6.1
				jQuery('.edit-post-post-template__toggle').on('DOMSubtreeModified', function(){
					var front_page_meta_boxes = jQuery('#frontpage_meta_box');

					if (jQuery(this).text() == 'Front-Page') {
						front_page_meta_boxes.removeClass('hidden');
					} else {
						front_page_meta_boxes.addClass('hidden');
					}
